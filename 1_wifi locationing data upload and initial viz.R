...
original_traindf <-
  read_csv(
    "C:\\Users\\minju\\Ubiqum\\Module 3\\indoorwifi-locationing\\UJIndoorLoc\\trainingData.csv"
  )
...
original_validdf <-
  read_csv(
    "C:\\Users\\minju\\Ubiqum\\Module 3\\indoorwifi-locationing\\UJIndoorLoc\\validationData.csv"
  )
...
p <-
  ggplot(countWAP_train, aes(Var1, Freq)) + geom_bar(stat = "identity", color = "white", fill = "#B2DF8A") +  xlab("WAP values in training dataset") + ylab("Frequency of occurrence")
p
...
q <-
  ggplot(countWAP_valid, aes(Var1, Freq)) + geom_bar(stat = "identity", color = "white", fill = "#1F78B4") +  xlab("WAP values in validation dataset") + ylab("Frequency of occurrence")
...
o <- ggplot(combinedcountWAP, aes(Var1, Freq, fill = source)) +
  geom_bar(stat = "identity",
           color = "white",
           position = "stack") + scale_fill_manual(
             values = c("#B2DF8A", "#1F78B4"),
             labels = c("WAPs in training dataset", "WAPs in validation dataset")
           ) + xlab("WAP values in training and validation datasets") + ylab("Frequency of occurrence") 
o
...
scatter3D(
  ...
  zlab = "Floor",
  bty = "b2",
  main = "Wifi indoor footprints (based on training data)",
  pch = 19,
  cex = 0.5,
  theta = -30,
  phi = 30,
...
scatter3D(
  ...
  zlab = "Floor",
  bty = "b2",
  main = "Wifi indoor footprints (based on validation data)",
  pch = 19,
  cex = 0.5,
  theta = -30,
  phi = 30,
